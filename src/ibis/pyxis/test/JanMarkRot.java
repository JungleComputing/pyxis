...
public class JanMarkRot {
	...
	// test values:
	// private static final float MIN_SX = 1;
	// private static final float MAX_SX = 3;
	// private static final float STEP_SX = 1;
	//
	// private static final float MIN_SY = 3;
	// private static final float MAX_SY = 9;
	// private static final float STEP_SY = 3;
	...
	static Image convRot(Image source, boolean adjustSize) throws Exception {
		...
		for (int rot = 0; rot < ROTATIONS; rot++) {
			...
			// STEP_THETA) {
			// FIXME not implemented (yet)
			Image rotatedImg = source.rotate(-theta, true, false, p);

			// NPO-version
			Image contrIm = rotatedImg.set(p).collectMax();
			// non-NPO version
			...
			Image backIm = contrIm.rotate(theta, true, false, p);
	...
	static Image convRotNoNPO(Image source, boolean adjustSize)
			throws Exception {
		int newSize = 0, beginX = 0, beginY = 0;
		int width = source.getWidth();
		int height = source.getHeight();
		...
		for (int rot = 0; rot < ROTATIONS; rot++) {
			...
			// Image contrIm = rotatedImg.set(p).collectMax();
			// non-NPO version
			Image contrIm = rotatedImg.set(p);
	...
	private static void viewImage(Image image, String text) {
		try {
			...
			outputImage = Imaging4j.convert(
					Imaging4j.convert(outputImage, Format.PYXIS_FLOATARGB),
					Format.ARGB32);
	...
	private static void saveImage(Image image, String filename)
		...
		// outputJpeg = Imaging4j.convert(Imaging4j.convert(outputImage,
		// Format.PYXIS_FLOATARGB), Format.ARGB32);
...
//				result = convRotNoNPO(image, false);
